/*
* Provides Carnival standardized tooltips as well as custom versions.
*
* DEPENDENCIES
*  - jQuery.1.4.2.min.js
* IMPLEMENTED
*  - 01-18-11
* DEVELOPERS
*  - jfolland@carnival.com, nsingh@carnival.com, amakhija@carnival.com
* DOCUMENTATION
...
(function (f) {
    // CCLQTip
    f.fn.cclqtip = function (asTitleContent, asContent, asTargetPosition, asTooltipPosition, asWidth, asHeight, aiXOffset, aiYOffset, asHTMLAttributes) {
        var loTooltipObject =
        {
            content: {
                text: '<span style="color:#898989" >Title</span>'
                //                title: {
                //                    text: '<img class="tooltip" style="margin-right: 10px" src="/cms/pageelements/2010/new-header/img/icon-help13x13.gif" alt="" /><span style="font-weight: bold;color:#0f65b8" >Text</span>'
                //                }
            },
        ...
        if (asTitleContent) {
            loTooltipObject.content.title = { text: '<img class="tooltip" style="margin-right: 10px" src="/cms/pageelements/2010/new-header/img/icon-help13x13.gif" alt="" /><span style="font-weight: bold;color:#0f65b8" >Text</span>' };
            loTooltipObject.content.title.text = asTitleContent;
        }
        if (asHTMLAttributes) {
            if (asHTMLAttributes.padding) {
                loTooltipObject.style.padding = asHTMLAttributes.padding;
            }
            if (asHTMLAttributes.margin) {
                loTooltipObject.style.margin = asHTMLAttributes.margin;
            }
        }
        loTooltipObject.content.text = asContent;
        if (asWidth) {
            loTooltipObject.style.width = asWidth;
        }
        if (asHeight) {
            loTooltipObject.style.height = asHeight;
        }
        if (asTargetPosition) {
            loTooltipObject.position.corner.target = asTargetPosition;
        }
        if (asTooltipPosition) {
            loTooltipObject.position.corner.tooltip = asTooltipPosition;
        }
        if (aiXOffset) {
            loTooltipObject.position.adjust.x = eval(aiXOffset);
        }
        if (aiYOffset) {
            loTooltipObject.position.adjust.y = eval(aiYOffset);
        }
        $(this).qtip(loTooltipObject);
    };
    // QTip
    $.fn.qtip = function (options, blanket) {
        ...
        if (typeof options == 'string') {
            ...
            if ($.isPlainObject($(this).data('qtip'))) $.fn.qtip.log.error.call(self, 1, $.fn.qtip.constants.NO_TOOLTIP_PRESENT, false);
        ...
        if (typeof options.content !== 'object' || (options.content.jquery && options.content.length > 0)) options.content =
            {
        ...
        if (typeof options.position.corner !== 'object') options.position.corner =
            {
        ...
        opts = $.extend(true, {
    }, $.fn.qtip.defaults, options);
    // Inherit all style properties into one syle object and include original options
    opts.style = buildStyle.call(
            {
                options: opts
            }, opts.style);
    opts.user = $.extend(true, {
}, options);
};
...
return $(this).each(function () // Return original elements as per jQuery guidelines
{
    ...
    if (typeof options == 'string') {
        ...
        if (typeof interfaces == 'object') {
            ...
            if (blanket === true && command == 'destroy') while (interfaces.length > 0) interfaces[interfaces.length - 1].destroy();
    ...
    for (i = 0; i < id; i++) {
        try { if (typeof $(this).data('qtip') === 'object' && $(this).data('qtip')); } catch (aoXcp) { }
    };
...
function qTip(target, options, id) {
    ...
    $.extend(self, self.options.api, {
        ...
        updatePosition: function (event, animate) {
            ...
            if (self.options.position.target !== 'mouse') {
                ...
                // Target is a regular HTML element, find position normally
                    ...
                    if (typeof self.options.position.target.attr('qtip') !== 'undefined') target.position = self.options.position.target.qtip('api').cache.position;
                    else
                ...
                if ((/right/i).test(target.corner)) newPosition.left += target.dimensions.width;
                if ((/bottom/i).test(target.corner)) newPosition.top += target.dimensions.height;
                if ((/((top|bottom)Middle)|center/).test(target.corner)) newPosition.left += (target.dimensions.width / 2);
                if ((/((left|right)Middle)|center/).test(target.corner)) newPosition.top += (target.dimensions.height / 2);
            }
            ...
            // Calculate correct target corner position
            if ((/right/i).test(tooltip.corner)) newPosition.left -= tooltip.dimensions.width;
            if ((/bottom/i).test(tooltip.corner)) newPosition.top -= tooltip.dimensions.height;
            if ((/((top|bottom)Middle)|center/).test(tooltip.corner)) newPosition.left -= (tooltip.dimensions.width / 2);
            if ((/((left|right)Middle)|center/).test(tooltip.corner)) newPosition.top -= (tooltip.dimensions.height / 2);
            // Setup IE adjustment variables (Pixel gap bugs)
            ieAdjust = ($.browser.msie) ? 1 : 0; // And this is why I hate IE...
            ie6Adjust = ($.browser.msie && parseInt($.browser.version.charAt(0)) === 6) ? 1 : 0; // ...and even more so IE6!
            // Adjust for border radius
            if (self.options.style.border.radius > 0) {
                if ((/Left/i).test(tooltip.corner)) newPosition.left -= self.options.style.border.radius;
                else if ((/Right/i).test(tooltip.corner)) newPosition.left += self.options.style.border.radius;
                if ((/Top/i).test(tooltip.corner)) newPosition.top -= self.options.style.border.radius;
                else if ((/Bottom/i).test(tooltip.corner)) newPosition.top += self.options.style.border.radius;
            };
            // IE only adjustments (Pixel perfect!)
            if (ieAdjust) {
                if ((/top/i).test(tooltip.corner)) newPosition.top -= ieAdjust
                else if ((/bottom/i).test(tooltip.corner)) newPosition.top += ieAdjust
                if ((/left/i).test(tooltip.corner)) newPosition.left -= ieAdjust
                else if ((/right/i).test(tooltip.corner)) newPosition.left += ieAdjust
                if ((/leftMiddle|rightMiddle/).test(tooltip.corner)) newPosition.top -= 1
            };
            ...
            if (self.options.position.target === 'mouse' && self.options.position.adjust.mouse === true) {
                if (self.options.position.adjust.screen === true && self.elements.tip) mouseAdjust = self.elements.tip.attr('rel');
                else
        ...
        updateWidth: function (newWidth) {
            ...
            else if (newWidth != undefined && typeof newWidth !== 'number') return $.fn.qtip.log.error.call(self, 2, 'newWidth must be of type number', 'updateWidth');
        ...
        updateContent: function (content, reposition) {
            ...
            // Set position and zoom to defaults to prevent IE hasLayout bug
            if ($.browser.msie) self.elements.contentWrapper.children().css(
                {
                    zoom: 'normal'
                });
            ...
            if (images.length) {
                ...
                else images.bind('load error', function () {
            ...
            if (!self.status.focused && curIndex !== newIndex) {
                ...
                $('div.qtip[qtip]').not(self.elements.tooltip).each(function () {
                    if ($(this).qtip('api').status.rendered === true) {
                        ...
                        if (typeof elemIndex == 'number' && elemIndex > -1) $(this).css(
                            {
        ...
        disable: function (state) {
            // Make sure tooltip is rendered and if not, return
            if (!self.status.rendered) return $.fn.qtip.log.error.call(self, 2, $.fn.qtip.constants.TOOLTIP_NOT_RENDERED, 'disable');
            if (state) {
        ...
        getPosition: function () {
            ...
            if (show) self.elements.tooltip.css(
                {
                    visiblity: 'hidden'
                }).show();
...
function construct() {
    ...
    self.elements.contentWrapper = self.elements.wrapper.children('div:first').css(
        {
            background: self.options.style.background
        });
    self.elements.content = self.elements.contentWrapper.children('div:first').css(jQueryStyle(self.options.style));
    // Apply IE hasLayout fix to wrapper and content elements
    if ($.browser.msie) self.elements.wrapper.add(self.elements.content).css(
        {
            zoom: 1
        });
...
function createTip(corner) {
    ...
    if ((/top/).test(corner) && $.browser.msie && parseInt($.browser.version.charAt(0)) === 6) self.elements.tip.css(
        {
            marginTop: -4
        });
...
function positionTip(corner) {
    ...
    if ((/top|bottom/).test(corner)) {
        ...
        if ($.browser.msie) {
            if (parseInt($.browser.version.charAt(0)) === 6) positionAdjust = ((/top/).test(corner)) ? -3 : 1;
            else
                positionAdjust = ((/top/).test(corner)) ? 1 : 2;
        };
        if ((/Middle/).test(corner)) self.elements.tip.css(
            {
                left: '50%',
                marginLeft: -(self.options.style.tip.size.width / 2)
            });
        else if ((/Left/).test(corner)) self.elements.tip.css(
            {
                left: self.options.style.border.radius - ieAdjust
            });
        else if ((/Right/).test(corner)) self.elements.tip.css(
            {
                right: self.options.style.border.radius + ieAdjust
            });
        if ((/top/).test(corner)) self.elements.tip.css(
            {
                top: -positionAdjust
            });
        else
        ...
        // Adjustments for IE6 - 0.5px border gap bug
        if ($.browser.msie) positionAdjust = (parseInt($.browser.version.charAt(0)) === 6) ? 1 : ((/left/).test(corner) ? 1 : 2);
        if ((/Middle/).test(corner)) self.elements.tip.css(
            {
                top: '50%',
                marginTop: -(self.options.style.tip.size.height / 2)
            });
        else if ((/Top/).test(corner)) self.elements.tip.css(
            {
                top: self.options.style.border.radius - ieAdjust
            });
        else if ((/Bottom/).test(corner)) self.elements.tip.css(
            {
                bottom: self.options.style.border.radius + ieAdjust
            });
        if ((/left/).test(corner)) self.elements.tip.css(
            {
                left: -positionAdjust
            });
        else
...
function createTitle() {
    ...
    self.elements.title = $('<div class="' + self.options.style.classes.title + '">').css(jQueryStyle(self.options.style.title, true)).css(
        {
...
function assignEvents() {
    ...
    if (self.options.hide.when.event == 'inactive') {
        ...
        inactiveEvents = ['click', 'dblclick', 'mousedown', 'mouseup', 'mousemove', 'mouseout', 'mouseenter', 'mouseleave', 'mouseover', 'touchstart'];
...
function screenAdjust(position, target, tooltip) {
    ...
    if (adjust.left) {
        if (self.options.position.target !== 'mouse') adjustedPosition.left = target.position.left + target.dimensions.width;
        else
    ...
    // Tooltip overflows off the right side of the screen
        ...
        if (self.options.position.target !== 'mouse') adjustedPosition.left = target.position.left - tooltip.dimensions.width;
        else
    ...
    if (adjust.top) {
        if (self.options.position.target !== 'mouse') adjustedPosition.top = target.position.top + target.dimensions.height;
        else
    ...
    // Tooltip overflows off the bottom of the screen
        ...
        if (self.options.position.target !== 'mouse') adjustedPosition.top = target.position.top - tooltip.dimensions.height;
        else
...
function sanitizeStyle(style) {
    if (typeof style.tip !== 'object') style.tip =
        {
            corner: style.tip
        };
...
$(document).ready(function () {
    ...
    $(window).bind('resize scroll', function (event) {
        ...
        adjustTimer = setTimeout(function () {
            ...
            if (event.type === 'scroll') $.fn.qtip.cache.screen.scroll =
                {
                    left: $(window).scrollLeft(),
                    top: $(window).scrollTop()
                };
